            ...
            @else
                ...
                >
                    ...
                    @if($row->field == "product_belongstomany_category_relationship")
                        @php($categories = app($options->model)->orderBy('nombre','ASC')->where('parent_id',null)->get())
                        @foreach($categories as $category)
                            <option value="{{ $category->{$options->key} }}" @if(in_array($category->{$options->key}, $selected_values)) selected="selected" @endif>
                                {{ $category->{$options->label} }}
                            </option>
                            @php($subCategories = \App\Category::orderBy('nombre','ASC')->where('parent_id',$category->id)->get())
                            @if(count($subCategories))
                                <optgroup label="Subcategorías de {{ $category->nombre }}">
                                    @foreach($subCategories as $subcategory)
                                        <option value="{{ $subcategory->{$options->key} }}" @if(in_array($subcategory->{$options->key}, $selected_values)) selected="selected" @endif>
